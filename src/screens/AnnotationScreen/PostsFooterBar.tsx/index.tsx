...
// Takes a bunch of posts and displays them in the footer selection bar.
const PostFooterBar = ({ currentPost, posts, setDisplayCreateNewPost, setCurrentPost }: PostFooterBarProps) => {
        ...
        for (let post in posts) {
            items.push(
                <div className='h-full w-full flex max-w-xs p-2 ml-2 my-auto' style={{maxWidth: '130px', minWidth: '130px', height: '235px'}}>
                    { renderPostIcon(posts[post]) }
                </div>
            )
        }

        let _sposts: JSX.Element[] = []
